...
export function pagadorFacturasPage(opts: { user: User; facturas: Factura[]; toast?: string }): string {
  const lista = [...opts.facturas].sort((a, b) => b.fechaEmision.localeCompare(a.fechaEmision));
  ...
  const content = `
    ...
    <div class="panel">
      ...
      ${
        lista.length === 0
          ? `<div class="empty-note">Todavía no cargaste ninguna factura. Arrancá con "+ Cargar factura".</div>`
          : `<table>
